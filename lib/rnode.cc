...
{
    memcpy(&a_content[a_len], &m_pageid,   sizeof(m_pageid));   a_len += sizeof(m_pageid);
    memcpy(&a_content[a_len], &m_level,    sizeof(m_level));    a_len += sizeof(m_level);
    memcpy(&a_content[a_len], &m_parent,   sizeof(m_parent));   a_len += sizeof(m_parent);
    memcpy(&a_content[a_len], &m_usedSpace,sizeof(m_usedSpace));a_len += sizeof(m_usedSpace);
    for (int i=0; i<m_usedSpace; i++)
    {
        int elen=0;
