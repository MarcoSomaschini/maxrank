...
Rtree::Rtree(Memory& a_memory,
             const int a_dimen,
             const int a_maxNodeChild, const int a_maxLeafChild,
             const int a_minNodeChild, const int a_minLeafChild,
             const bool a_pointOnly):
m_memory(a_memory),
m_dimen(a_dimen),
m_maxNodeChild(a_maxNodeChild),
m_minNodeChild(a_minNodeChild),
m_maxLeafChild(a_maxLeafChild),
m_minLeafChild(a_minLeafChild),
            ...
            case NODE_OVERFLOW:
                {
                    ...
                    n->m_usedSpace=0;   // reset the usedspace since all
                                        // entries are moved to split nodes
...
int Rtree::remove(const RtreeNodeEntry& a_entry)
{
    ...
    public:
        carrier(const int a_level, RtreeNodeEntry* a_entry):
          m_level(a_level), m_entry(a_entry) {};
        virtual ~carrier(){};
    };
...
bool Rtree::integrityTest() const
{
    ...
    public:
        carrier(const int a_pageid, const int a_level=0, RtreeNodeEntry* a_e=0):
          m_pageid(a_pageid), m_level(a_level), m_e(a_e)
          {};
        virtual ~carrier() {};
    };
