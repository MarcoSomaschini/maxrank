...
int TGS::partition(Rtree &a_rtree,
                   RtreeNodeEntry **a_p, const int a_numEntry,
                   const int a_level, Array &a) {
    int r = 0;
    const int max =
            a_level == 0 ? a_rtree.m_maxLeafChild : a_rtree.m_maxNodeChild;
    if (a_numEntry <= max) {   // create a node for entries
        ...
        r = 1;
    } else {   // partition a set of entries into two
        RtreeNodeEntry **gp0 = new RtreeNodeEntry *[a_numEntry];
        RtreeNodeEntry **gp1 = new RtreeNodeEntry *[a_numEntry];
        int cnt0 = 0, cnt1 = 0;

        //a_p[0]->quadraticSplit(
        //    a_p, a_numEntry, a_numEntry/2, gp0, cnt0, gp1, cnt1);
        a_p[0]->packedSplit(
                a_p, a_numEntry, max, gp0, cnt0, gp1, cnt1);
...
Rtree *TGS::bulkload(Memory &a_memory,
                     const int a_dimen,
    ...
    Rtree *rtree = new Rtree(a_memory, a_dimen,
                             a_maxNodeFill, a_maxLeafFill, a_minNodeFill, a_minLeafFill,
                             a_pointOnly);
