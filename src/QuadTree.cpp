...
int PointVersusHalfSpace(const int &Dimen, const float hs[], const Point1 &pt);   //position of a point to halfspace: is the point above, below, or on the halfspace
...
void GenBinaryString(long int len1,long int Max, multimap<int,vector<char> >  &binString);  //function to generate combinations
void GenLenNBinaryString(long int len1,long int HammingDistance,multimap<int,vector<char> > &binString); //gen binstring with specified Hamming len
...
bool QuadTree::checkNodeValidity(const float Qspace[], QuadNode *node, vector<string> &Comb){    //check validity of node with respect to the subdivision; 
...
bool QuadTree::splitNode(QuadNode *node){
        ...
        if (node->Level == MaxLevel) return false;   //if the level of current node is exceeding the maximal level, then stop node-splitting
	...
	for (int i=0;i<numOfSubdivisions;i++){
	     ...
	     node->Child[i]->Level = node->Level+1;
             ...
             if (checkNodeValidity(Qspace,node->Child[i],Comb)==true)  //prune away nodes lying outside q_1+q_2+...q_{d-1}<=1
	...
	numOfLeafNodes = numOfLeafNodes + numOfSubdivisions - 1;    //aggregate the number of leaf nodes whenever a node-split happened
...
bool QuadTree::appendHalfSpace(const long hsID, QuadNode *node){   //append a single halfspace to Quadtree
    ...
    if (pos == BELOW){    //halfspace lies above current node, then store it into its list
	    ...
	    return true;
		        ...
		        if (n->Leaf == LEAFNODE){   //current node n is a leaf, so we need to insert hs to n's intersectedHS set and then check the capacity of the set
					(n->intersectedHalfspace).push_back(hsID);
					if ((n->intersectedHalfspace).size()>MaxCapacity) {   //after insertion, max capacity of the set intersectedHalfspace exceeded, we need to split current node
						if (n->Level < MaxLevel){  
                            splitNode(n);
							for (vector<long>::iterator itr=(n->intersectedHalfspace).begin();itr != (n->intersectedHalfspace).end(); itr++){
...
void QuadTree::collectLeafNodes(QuadNode *root, vector<std::pair<long,QuadNode*> > &Leaves, vector<long int> &numOfCoveredHS){
...
//
bool QuadTree::GenHammingHalfSpaces(char *OutFileName, const int Dimen, vector<char>& HammingString, vector<long int> &HalfSpaceIDs, float subDataSpace[])
{
	 ...
	 while (true)
	 {
		 ...
		 if (loops >= MAXLOOP) //there is a very low probability that an interior point exists for current set of halfspaces
		 { 
     ...
     for (int i=0;i<Dimen;i++) fprintf(fout1,"%f ",InteriorPt[i]); //the feasible point found by Monte Carlo process above   *****************
     fprintf(fout1,"\n"); 
...
long int QuadTree::naiveInNodeIntersection(vector<std::pair<long,QuadNode*> > &Leaves,vector<set<long int> > &minCellHalfSpaces, vector<vector<char> > &binaryString)   //a naive solution that exhaustively search all combinations of halfspaces (in a leaf node) to intersect
{
	...
	for (itr = Leaves.begin(); itr != Leaves.end();)    // && NotFoundAllMinCells;)
	{
		 ...
		 if (!isValid)
		 {
	         ...
	         if (NoOfCoveredHS - tao > minOrder) //terminate searching min-cells, cause no cell with smaller order exists any more
                 {
		 ...
		 for (multimap<long,QuadNode*>::iterator itr1 = nodesToIntersect.begin(); itr1 != nodesToIntersect.end(); itr1++)
		 {
			  ...
			  size_t substrPos1,substrPos2;	  
			     ...
			     {  	
			           ...
			           if (verbose)
			               cout << "Hamming Dist="<< HammingDistance << ", Mininmal Order=" << minOrder << ", #coveredHS="<< NoOfCoveredHS << ", #intersectedHS="<< NoOfHalfSpacesInNode << endl;
			           if (verbose) 
			               cout << "Testing Hamming string: " << string(itrHamming->second.begin(),itrHamming->second.end()) << endl;			   			           
				   ...
				   bool nonZeroExtent = GenHammingHalfSpaces(halfspaceFileName,Dimen, itrHamming->second, itr1->second->intersectedHalfspace, itr1->second->MBR);                                  				   
				   ...
				   FilesToRemove.push_back(volumeFilename);  //collect the files for removal later
                                   ...
                                   if (volume > ZEROEXTENT) //ZEROEXTENT: 1e-10
                                   {
                                       ...
                                       {                                           
                                           ...
                                           set<long int> tmpSet;
                                           std::copy((itr1->second->intersectedHalfspace).begin(),(itr1->second->intersectedHalfspace).end(),std::inserter(tmpSet,tmpSet.begin()));
                                           ...
                                           if (verbose)
                                               cout << "Node: " << itr1->second->NodeID << ", found a min-cell with binstring:"<< string(itrHamming->second.begin(),itrHamming->second.end()) << endl;                       
                                       } else
                                       { 
                                           NoOfDiscardedCells++;
                                           stopIncrHammingDist=true;                                           
				           ...
				           FilesToRemove.push_back(tmpString);  //collect the files for removal 
                                   ...
                                   } else 
                                   {
                                       NoOfZeroExtentBinStrings++;
				       if (verbose)
				           cout <<"Node: "<< itr1->second->NodeID<<", HammingDist: "<<HammingDistance<< " is empty!"<< endl;
				   }				   
				   if (verbose) 
	...
	if (verbose) cout << "#min-Cells found: " << minCellHalfSpaces.size() << ", minOrder="<< minOrder+NoOfCoveredHS << endl;
...
long int QuadTree::optimizedInNodeIntersection(vector<std::pair<long,QuadNode*> > &Leaves,vector<set<long int> > &minCellHalfSpaces, vector<vector<char> > &binaryString) //optimization of within node intersection
{
        multimap<long int, QuadNode *> nodesToIntersect;   //store the nodes (in ascending order) by using the number of their intersected halfspaces
	//multimap<long int, QuadNode *>::iterator itr, itr1;
	vector<std::pair<long,QuadNode*> >::iterator itr, itr1;
	...
	for (itr = Leaves.begin(); itr != Leaves.end();)    // && NotFoundAllMinCells;)
	{
		 ...
		 if (!isValid)
		 {
	         ...
	         NoOfCoveredHS = (*itr).first;
	         if (NoOfCoveredHS > minOrder) break; //terminate searching min-cells, cause no cell with smaller order exists any more 
		 ...
		 for (multimap<long,QuadNode*>::iterator itr1 = nodesToIntersect.begin(); itr1 != nodesToIntersect.end(); itr1++)
		 {
			  ...
			  if (NoOfHalfSpacesInNode <= 1 ) continue;
                          ...
                          for (vector<long>::iterator sItr=(itr1->second->intersectedHalfspace).begin();sItr!=(itr1->second->intersectedHalfspace).end();sItr++)
                          {
                               long int hs1=(*sItr);  
...
bool QuadTree::testHalfspacePair(long int HS1,long int IdxHS1,long int HS2, long int IdxHS2, float subDataSpace[], multimap<int, string>& InValidHammingStr) //test whether two halfspaces are compatible w.r.t Hamming distance 00,01,10,11
{
	 ...
	 while (true)
	 {
		 ...
		 break;
             ...
             for (int j = 0; j < Dimen; j++)	     
	          InteriorPt[j] = subDataSpace[j] + (subDataSpace[Dimen+j]-subDataSpace[j])*(float(rand())/RAND_MAX);
...
bool QuadTree::readCombinations(){
	...
	char m_separator[] = " \n\t";
	char buf[512];
	string FileName[]={"Comb2D.txt","Comb3D.txt","Comb4D.txt","Comb5D.txt","Comb6D.txt","Comb7D.txt","Comb8D.txt","Comb9D.txt"};
	string strComb;
