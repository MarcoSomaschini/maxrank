        ...
        int& maxStackSize, Array& a_pageaccessed)
{
    ...
    float dataspace[4][2]={{0,0},{1,0},{1,1},{0,1}};
        ...
        for (;c_Itr1!=Cells.end();)  
        {
             ...
             { 
                 ...
                 while (c_Itr2!=Cells.end())
                 {
                     ...
                     {
                         long int tmpNoOfSingulars=0;
                         for (sItr=c_Itr2->second->HalflineL.begin();sItr!=c_Itr2->second->HalflineL.end();sItr++)
                         {
                              sItr1=Singular.find((*sItr));
                              if (sItr1!=Singular.end()) tmpNoOfSingulars++;
                         }
    ...
    cout << "minimal k= " << min_k << ", NoOfDominators=" << NoOfDominators << ", NoOfDominees="<< NoOfDominees << endl;
        ...
        int& maxStackSize, Array& a_pageaccessed)
{
    ...
    NoOfDominators=incomparableWindowsHD(a_rtree,PG,Dominee_hc,Dominator_hc,NonResultEntry,ResultID,maxStackSize,a_pageaccessed);
    //if (verbose) 
    ...
    {    
        ...
        if (verbose)
            cout << "Number of NonResultEntry: "<< NonResultEntry.size() << ", Number of index nodes:"<< ResultID.size() << endl;        
        ...
        if (verbose)
            cout<<"m_k="<<m_k<<", min_k="<<min_k<<", #tmpMinCells: "<<minCellHalfSpaces.size()<<", #dominators= "<<NoOfDominators<<endl;
        //getchar();        
        ...
        for (int i = 0; i < tmpCellCnt; i++)  //examine each interim minCells
        {
           ...
           for (sItr = minCellHalfSpaces[i].begin(); sItr != minCellHalfSpaces[i].end(); sItr++)
           {
             ...
             if (iRes.second==true)   //current halfline with id *sItr is augmented, so remove it from the skyline set
    ...
    if (verbose) cout << "#minCells with order " << min_k << ": "<< NoOfMinCells << ", #Dominators="<< NoOfDominators << endl;
    //getchar();
        ...
        int& maxStackSize, Array& a_pageaccessed)
{
   ...
   //compute the number of records whose score is greater/smaller than S(a_pt) within each intervals
    ...
    for (int i=0; i<4; i++)
    {
      ...
      for (vecItr=ResultID[i].begin();vecItr!=ResultID[i].end();vecItr++)  
      {
         ...
         if (mDblItr != RdAndCrossPt.end())
         {
             ...
             else if (rd[1] > a_pt[1])//(rd[0] < a_pt[0])  //current record will score higher than a_pt in interval [0,crosspoint)
             {         
    ...
    //*/
    cout << "minimal k= " << min_k+NoOfDominators << ", maximum k=" << max_k+ NoOfDominators << " NoOfDominators=" << NoOfDominators << ", NoOfDominees="<< NoOfDominees << endl;
        ...
        int& maxStackSize, Array& a_pageaccessed)
{
    ...
    NoOfDominators=getIncomparableRecords(a_rtree,PG, Dominee_hc,Dominator_hc, ResultID, maxStackSize,a_pageaccessed);
        ...
        float* PG[])
{
     ...
     for (i=0;i<dimen;i++) fprintf(fout1,"%f ",a_query.function[i]); //the feasible point is the query point itself
     fprintf(fout1,"\n"); 
