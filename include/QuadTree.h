...
public:
    ...
    bool checkNodeValidity(const float Qspace[], QuadNode *node,
    ...
    void
    collectLeafNodes(QuadNode *root, vector<std::pair<long, QuadNode *> > &Leaves, vector<long int> &listOfCoveredHS);
    ...
    long int
    naiveInNodeIntersection(vector<std::pair<long, QuadNode *> > &Leaves, vector<set<long int> > &minCellHalfSpaces,
    ...
    long int
    optimizedInNodeIntersection(vector<std::pair<long, QuadNode *> > &Leaves, vector<set<long int> > &minCellHalfSpaces,
                                vector<vector<char> > &binaryString);//optimization of within node intersection
    bool testHalfspacePair(long int HS1, long int IdxHS1, long int HS2, long int IdxHS2, float subDataSpace[],
